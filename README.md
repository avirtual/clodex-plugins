...
| [`ntfy`](ntfy/) | Subscribes to an [ntfy](https://ntfy.sh) topic — a GitHub webhook, say — and raises each message as an operator inbox note and optionally a DM to one seat, fenced as untrusted. **The one with a test suite**, run against the real host engine and a real socket. Needs Clodex ≥ 5.36.0 for `host.notify.user`. |

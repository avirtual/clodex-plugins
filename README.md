...
| [`ntfy`](ntfy/) | Subscribes to [ntfy](https://ntfy.sh) topics — GitHub webhooks, say — and raises each message as an operator inbox note and optionally a rate-limited DM to a seat, fenced as untrusted. Several topics on one server, each with its own seat. **The one with a test suite**, run against the real host engine and a real socket. Needs Clodex ≥ 5.36.0 for `host.notify.user`. |
